...
        }
        /* Transitions for Request Modal */
        #request-confirmation-modal {
            transition: opacity 0.3s ease-in-out;
        }
        #request-confirmation-modal > div {
            transition: transform 0.3s ease-in-out, opacity 0.3s ease-in-out;
        }
    `;
    document.head.appendChild(style);

    // INJECT REQUEST CONFIRMATION MODAL
    const requestModalHTML = `
    <div id="request-confirmation-modal" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center hidden z-50 p-4 opacity-0">
        <div class="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-md text-white relative transform scale-95 opacity-0">
            <button data-action="close-request-modal" class="absolute top-2 right-3 text-gray-400 hover:text-white text-3xl leading-none font-bold">&times;</button>
            <h3 class="text-xl font-bold mb-4">Confirmar Pedido</h3>
            <div id="request-modal-content" class="flex items-start gap-4 mb-6 min-h-[100px]">
                <!-- Content will be populated by JS -->
            </div>
            <div class="flex justify-end gap-4">
                <button data-action="close-request-modal" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg transition">Cancelar</button>
                <button id="confirm-request-btn" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg transition">Solicitar</button>
            </div>
        </div>
    </div>
    `;
...
    }
    ...
    async function showRequestConfirmationModal(tmdbId, mediaType) {
        ...
        modalContent.innerHTML = `<div class="w-full h-24 bg-gray-700 rounded-md animate-pulse"></div>`; // Placeholder
        ...
        try {
            ...
            modalContent.innerHTML = `
                <img src="${TMDB_IMG_URL}${data.poster_path}" alt="${title}" class="w-20 h-30 object-cover rounded-md flex-shrink-0">
                <div>
                    <h4 class="font-bold">${title} (${year})</h4>
                    <p class="text-sm text-gray-400 mt-1">${overview}</p>
                </div>
            `;
            
            confirmBtn.onclick = () => createRequest(tmdbId, mediaType);

        } catch (error) {
            console.error("Erro ao buscar detalhes no TMDB para o modal:", error);
            modalContent.innerHTML = `<p class="text-red-400">Não foi possível carregar os detalhes do item. Tente novamente.</p>`;
            confirmBtn.onclick = null;
        }
    }

    function closeRequestConfirmationModal() {
        const modal = document.getElementById('request-confirmation-modal');
        const modalDialog = modal.querySelector('div');
...
        }
        
        await showRequestConfirmationModal(tmdbId, mediaType);
    }

    async function createRequest(tmdbId, mediaType) {
        closeRequestConfirmationModal(); // Close modal immediately
        const profile = getCurrentProfile();
...
        const nonPreventActions = ['close-trailer-modal', 'handleNotificationClick', 'closeNotifications', 'dismiss-notification', 'toggleCastVisibility', 'showReplyForm', 'add-reply', 'like', 'delete', 'rate', 'show-more-comments', 'removeFromContinueWatching', 'remove-my-request', 'close-request-modal'];
